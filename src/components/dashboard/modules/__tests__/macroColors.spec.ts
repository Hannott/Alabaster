...
describe('macroColorKey / macroColorVariable', () => {
  it('defaults to no colour, unlike a sensor which always has one', () => {
    expect(macroColorKey('CLEAN_NOZZLE', {})).toBeNull()
    expect(macroColorVariable('CLEAN_NOZZLE', {})).toBeNull()
  })

  it('resolves a chosen colour to its CSS variable', () => {
  ...
  it('degrades a stale or hand-edited value to no colour rather than crashing', () => {

...
single-printer install it is not there at all, and you can turn it off from
[Settings → Printers](/guide/printers#seeing-them-all-at-once) if you would
rather work one machine at a time.

## The rail

Printers sit side by side in one row that scrolls sideways. Five fit on a
1920-wide screen with the next one part way on, and adding more never re-shapes
the ones already on screen — they simply extend to the right. The row scrolls, the page does not: use the
scrollbar, drag, or click the rail and use <kbd>←</kbd> and <kbd>→</kbd>.
<kbd>Home</kbd> and <kbd>End</kbd> jump to either end.
...
Expanding never changes what Alabaster is connected to, so it is instant — and
...
A collapsed column already carries everything that acts on the machine,
including an emergency stop in its top corner — that one is there because the
reason to reach for it is spotting trouble on a machine you were not driving.
...
| **Go to dashboard**    | On the card Alabaster is already driving — the one that leaves for that printer's dashboard.                               |
...
file to the queue — or, on a machine with nothing loaded, start it straight
away. Alabaster asks before starting a print on a machine you are not standing
at, and names it.

For everything else a file needs — thumbnails, slicer data, uploading, deleting
— **Switch** to the printer and use [Print files](/interface/print-files).
